...
public final class GCMRegistrar {
	...
	public static final String PROPERTY_SENDERID = "senderID";
	public static final String PROPERTY_TAGCLASS = "tagClass";
	 ...
	 *             if the device does not support GCM.
	 */
	 ...
	 * {@value com.google.android.gcm.GCMConstants#INTENT_FROM_GCM_MESSAGE} and
	 ...
	 *             if any of the conditions above is not met.
	 */
	 ...
	 */
	public static boolean register(Context context, String... senderIds) {
		GCMRegistrar.resetBackoff(context);
		internalRegister(context, senderIds);
		return true;
	}

	static boolean  internalRegister(Context context, String... senderIds) {
		String flatSenderIds=null;
		try{
			flatSenderIds = getFlatSenderIds(senderIds);
		}catch(Exception e)
		{
			Log.e("", "internalRegister Error");
			return false;
		}
	...
	static String getFlatSenderIds(String... senderIds) throws Exception {
	...
	static synchronized void setRetryBroadcastReceiver(Context context) {
		if (sRetryReceiver == null) {
			if (sRetryReceiverClassName == null) {
				// should never happen
				Log.i(TAG, "internal error: retry receiver class not set yet");
	...
	public static void setSenderId(Context context, String senderId,Class<?> targetClass) {
		final SharedPreferences prefs = getGCMPreferences(context);
		Editor editor = prefs.edit();
		editor.putString(PROPERTY_SENDERID, senderId);
		editor.putString(PROPERTY_TAGCLASS, targetClass.getName());
