...
public abstract class GCMBaseIntentService extends IntentService {
	...
	private static final int MAX_BACKOFF_MS = (int) TimeUnit.SECONDS.toMillis(3600); // 1
																						// hour
	 ...
	 * @throws Exception 
	 */
	protected GCMBaseIntentService(String... senderIds) throws Exception {
	...
	private static String getName(String[] senderIds) throws Exception {
	 ...
	 *             if sender id was not set on constructor.
	 */
	protected String[] getSenderIds(Context context) {
		if (mSenderIds == null) {
			final SharedPreferences prefs =GCMRegistrar.getGCMPreferences(context);
			mSenderIds[0]=prefs.getString(GCMRegistrar.PROPERTY_SENDERID, null);
			try {
	...
	protected abstract void onRegisteredFailed();
	...
	static void runIntentInService(Context context, Intent intent, String className) {
		...
		//	e.printStackTrace();
			Log.e(TAG, "runIntentInService failed!");
		}
	}
		...
		if (ERROR_SERVICE_NOT_AVAILABLE.equals(error)) {
			...
			if (retry) {
				...
				if (backoffTimeMs < MAX_BACKOFF_MS) {
					...
					Log.w(TAG, "Not retrying failed operation");
			}
		} else {
