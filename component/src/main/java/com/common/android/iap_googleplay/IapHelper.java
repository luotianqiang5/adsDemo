...
import com.common.android.OnConsumeListener;
...
public class IapHelper {
	...
	public boolean IAPSuppored = false;
	 ...
	 */
	public void startSetup(final OnSetupListener listener) {
		...
		mServiceConn = new ServiceConnection() {
			...
			public void onServiceConnected(ComponentName name, IBinder service) {
				...
				try {
					...
					if (response != ResponseCode.BILLING_RESPONSE_RESULT_OK) {
						if (listener != null)
							listener.onIabSetupFinished(new IapResult(response, "Error checking for billing v3 support."));
					...
					if (listener != null) {
						listener.onIabSetupFinished(new IapResult(IABHELPER_REMOTE_EXCEPTION, "RemoteException while setting up in-app billing."));
					}
				//	e.printStackTrace();
					IAPSuppored = false;
					return;
				...
				if (listener != null) {
					listener.onIabSetupFinished(new IapResult(ResponseCode.BILLING_RESPONSE_RESULT_OK, "Setup successful."));
				}
			}
		};
		...
		if (!mContext.getPackageManager().queryIntentServices(serviceIntent, 0).isEmpty()) {
			...
			IAPSuppored = false;
			if (listener != null) {
				listener.onIabSetupFinished(new IapResult(ResponseCode.BILLING_RESPONSE_RESULT_BILLING_UNAVAILABLE, "Billing service unavailable on device."));
			}
		}
	}
	...
	public void launchPurchaseFlow(Activity act, String sku, int requestCode, OnPurchaseListener listener) {
		launchPurchaseFlow(act, sku, requestCode, listener, "");
	}

	public void launchPurchaseFlow(Activity act, String sku, int requestCode, OnPurchaseListener listener, String extraData) {
		launchPurchaseFlow(act, sku, ITEM_TYPE_INAPP, requestCode, listener, extraData);
	}

	public void launchSubscriptionPurchaseFlow(Activity act, String sku, int requestCode, OnPurchaseListener listener) {
		launchSubscriptionPurchaseFlow(act, sku, requestCode, listener, "");
	}

	public void launchSubscriptionPurchaseFlow(Activity act, String sku, int requestCode, OnPurchaseListener listener, String extraData) {
	 ...
	 */
	public void launchPurchaseFlow(final Activity act, String sku, String itemType, int requestCode, OnPurchaseListener listener, String extraData) {
		IapResult result;

		logDebug("---------Security----------:"+mSignatureBase64);
		if (mDebugLog)
		{
			act.runOnUiThread(new Runnable() {
				@Override
				public void run() {
					Toast.makeText(act.getApplicationContext(),"========IAP Debug========",Toast.LENGTH_SHORT).show();
					
				}
			});
			result = new IapResult(ResponseCode.BILLING_RESPONSE_RESULT_OK, null);
			PurchaseBean p=new PurchaseBean(sku);
			if (listener != null)
				listener.onIabPurchaseFinished(result, p);
			return;
		}else if (!mSetupDone || !IAPSuppored) {
			logError("Illegal state for operation (launchPurchaseFlow): IAB helper is not set up.");
			result = new IapResult(ResponseCode.BILLING_RESPONSE_RESULT_BILLING_UNAVAILABLE, null);
			if (listener != null)
				listener.onIabPurchaseFinished(result, null);
			return;
		}
		if (!flagStartAsync("launchPurchaseFlow")) {
			result = new IapResult(ResponseCode.BILLING_RESPONSE_RESULT_UNDERWAY, null);
			if (listener != null) {
				listener.onIabPurchaseFinished(result, null);
				return;
			}
		...
		if (itemType.equals(ITEM_TYPE_SUBS) && !mSubscriptionsSupported) {
			IapResult r = new IapResult(IABHELPER_SUBSCRIPTIONS_NOT_AVAILABLE, "Subscriptions are not available.");
			if (listener != null)
		...
		try {
			...
			if (response != ResponseCode.BILLING_RESPONSE_RESULT_OK) {
				...
				result = new IapResult(response, "Unable to buy item");
				if (listener != null)
					listener.onIabPurchaseFinished(result, null);
				return;
			}
			...
			result = new IapResult(IABHELPER_SEND_INTENT_FAILED, "Failed to send intent.");
			if (listener != null)
				listener.onIabPurchaseFinished(result, null);
		} catch (RemoteException e) {
			...
			result = new IapResult(IABHELPER_REMOTE_EXCEPTION, "Remote exception while starting purchase flow");
			if (listener != null)
				listener.onIabPurchaseFinished(result, null);
		}
	}
...
/**
	...
	public boolean handleActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (data == null) {
			...
			result = new IapResult(IABHELPER_BAD_RESPONSE, "Null data in IAB result");
		...
		if (resultCode == Activity.RESULT_OK && responseCode == ResponseCode.BILLING_RESPONSE_RESULT_OK) {
			...
			if (purchaseData == null || dataSignature == null) {
				...
				result = new IapResult(IABHELPER_UNKNOWN_ERROR, "IAB returned null purchaseData or dataSignature");
				if (mPurchaseListener != null)
					mPurchaseListener.onIabPurchaseFinished(result, null);
				return true;
			}
			...
			try {
				purchase = new PurchaseBean(mPurchasingItemType, purchaseData, dataSignature);
				String sku = purchase.getSku();
				// Verify signature
				if (!Security.verifyPurchase(mSignatureBase64, purchaseData, dataSignature)) {
					logError("Purchase signature verification FAILED for sku " + sku);
					result = new IapResult(IABHELPER_VERIFICATION_FAILED, "Signature verification failed for sku " + sku);
					if (mPurchaseListener != null)
				...
				e.printStackTrace();
				result = new IapResult(IABHELPER_BAD_RESPONSE, "Failed to parse purchase data.");
				if (mPurchaseListener != null)
					mPurchaseListener.onIabPurchaseFinished(result, null);
				return true;
			}
			...
			if (mPurchaseListener != null) {
				mPurchaseListener.onIabPurchaseFinished(new IapResult(ResponseCode.BILLING_RESPONSE_RESULT_OK, "Success"), purchase);
			}
		} else if (resultCode == Activity.RESULT_OK) {
			// result code was OK, but in-app billing response was not OK.
			logDebug("Result code was OK but in-app billing response was not OK: " + getResponseDesc(responseCode));
			if (mPurchaseListener != null) {
				result = new IapResult(responseCode, "Problem purchashing item.");
				mPurchaseListener.onIabPurchaseFinished(result, null);
			}
		}/*
		 * else if (resultCode == Activity.RESULT_CANCELED) {
		 * logDebug("Purchase canceled - Response: " +
		 * getResponseDesc(responseCode)); result = new
		 * IabResult(IABHELPER_USER_CANCELLED, "User canceled."); if
		 * (mPurchaseListener != null)
		 * mPurchaseListener.onIabPurchaseFinished(result, null); }
		 */else {
			logError("Purchase failed. Result code: " + Integer.toString(resultCode) + ". Response: " + getResponseDesc(responseCode));
			//			result = new IabResult(IABHELPER_UNKNOWN_PURCHASE_RESPONSE, "Unknown purchase response.");
			result = new IapResult(responseCode, null);
			if (mPurchaseListener != null)
				mPurchaseListener.onIabPurchaseFinished(result, null);
		}
		return true;
	...
	public Inventory queryInventory(boolean querySkuDetails, List<String> moreSkus) throws IapException {
	 ...
	 */
	public Inventory queryInventory(boolean querySkuDetails, List<String> moreItemSkus, List<String> moreSubsSkus) throws IapException {
		checkSetupDone("queryInventory");
		if(!IAPSuppored)
			throw new IapException(ResponseCode.BILLING_RESPONSE_RESULT_BILLING_UNAVAILABLE, getResponseDesc(ResponseCode.BILLING_RESPONSE_RESULT_BILLING_UNAVAILABLE));
		
		try {
			Inventory inv = new Inventory();
			int r = queryPurchases(inv, ITEM_TYPE_INAPP);
			if (r != ResponseCode.BILLING_RESPONSE_RESULT_OK) {
				throw new IapException(r, "Error refreshing inventory (querying owned items).");
			}

			if (querySkuDetails) {
				r = querySkuDetails(ITEM_TYPE_INAPP, inv, moreItemSkus);
				if (r != ResponseCode.BILLING_RESPONSE_RESULT_OK) {
					throw new IapException(r, "Error refreshing inventory (querying prices of items).");
				}
			}
			...
			if (mSubscriptionsSupported) {
				...
				if (r != ResponseCode.BILLING_RESPONSE_RESULT_OK) {
					throw new IapException(r, "Error refreshing inventory (querying owned subscriptions).");
				}

				if (querySkuDetails) {
					r = querySkuDetails(ITEM_TYPE_SUBS, inv, moreItemSkus);
					if (r != ResponseCode.BILLING_RESPONSE_RESULT_OK) {
						throw new IapException(r, "Error refreshing inventory (querying prices of subscriptions).");
					}
				}
			}
			...
			throw new IapException(IABHELPER_REMOTE_EXCEPTION, "Remote exception while refreshing inventory.", e);
		} catch (JSONException e) {
			throw new IapException(IABHELPER_BAD_RESPONSE, "Error parsing JSON response while refreshing inventory.", e);
		}
	}
	 ...
	 */
	public void queryInventoryAsync(final boolean querySkuDetails, final List<String> moreSkus, final OnQueryListener listener) {
		final Handler handler = new Handler(mContext.getMainLooper());
		checkSetupDone("queryInventory");
		flagStartAsync("refresh inventory");
		(new Thread(new Runnable() {
			public void run() {
				IapResult result = new IapResult(ResponseCode.BILLING_RESPONSE_RESULT_OK, "Inventory refresh successful.");
				Inventory inv = null;
				try {
					inv = queryInventory(querySkuDetails, moreSkus);
				} catch (IapException ex) {
					result = ex.getResult();
				}

				flagEndAsync();

				final IapResult result_f = result;
	...
	public void queryInventoryAsync(OnQueryListener listener) {
		queryInventoryAsync(true, null, listener);
	}

	public void queryInventoryAsync(boolean querySkuDetails, OnQueryListener listener) {
	 ...
	 */
	void consume(PurchaseBean itemInfo) throws IapException {
		checkSetupDone("consume");

		if (!itemInfo.mItemType.equals(ITEM_TYPE_INAPP)) {
			throw new IapException(IABHELPER_INVALID_CONSUMPTION, "Items of type '" + itemInfo.mItemType + "' can't be consumed.");
		}
		...
		try {
			...
			if (token == null || token.equals("")) {
				...
				throw new IapException(IABHELPER_MISSING_TOKEN, "PurchaseInfo is missing token for sku: " + sku + " " + itemInfo);
			}
			...
			if (response == ResponseCode.BILLING_RESPONSE_RESULT_OK) {
				...
				throw new IapException(response, "Error consuming sku " + sku);
			}
		} catch (RemoteException e) {
			throw new IapException(IABHELPER_REMOTE_EXCEPTION, "Remote exception while consuming. PurchaseInfo: " + itemInfo, e);
		}
	}
	 ...
	 *            finishes.
	 */
	public void consumeAsync(PurchaseBean purchase, OnConsumeListener listener) {
		checkSetupDone("consume");
	...
	public static String getResponseDesc(int code) {
		String[] iab_msgs = ("0:OK/1:User Canceled/2:Unknown/" + "3:Billing Unavailable/4:Item unavailable/" + "5:Developer Error/6:Error/7:Item Already Owned/"
				+ "8:Item not owned/9:Other Operation Underway/10:Network Error/11:Billing not ready").split("/");
		String[] iabhelper_msgs = ("0:OK/-1001:Remote exception during initialization/" + "-1002:Bad response received/" + "-1003:Purchase signature verification failed/"
				+ "-1004:Send intent failed/" + "-1005:User cancelled/" + "-1006:Unknown purchase response/" + "-1007:Missing token/" + "-1008:Unknown error/"
	...
	int queryPurchases(Inventory inv, String itemType) throws JSONException, RemoteException {
		...
		do {
			...
			for (int i = 0; i < purchaseDataList.size(); ++i) {
				...
				if (Security.verifyPurchase(mSignatureBase64, purchaseData, signature)) {
					...
					PurchaseBean purchase = new PurchaseBean(itemType, purchaseData, signature);
		...
		return verificationFailed ? IABHELPER_VERIFICATION_FAILED : ResponseCode.BILLING_RESPONSE_RESULT_OK;
	}
	...
	void consumeAsyncInternal(final PurchaseBean purchase, final OnConsumeListener multiListener) {
		final Handler handler = new Handler(mContext.getMainLooper());
		...
		(new Thread(new Runnable() {
			...
			public void run() {
				try {
					consume(purchase);
					result=new IapResult(ResponseCode.BILLING_RESPONSE_RESULT_OK, "Successful consume of sku " + purchase.getSku());
				} catch (IapException ex) {
					result=ex.getResult();
				}
				...
				if (multiListener != null) {
					handler.post(new Runnable() {
						...
						public void run() {
							multiListener.onConsumeFinished(purchase, result);
						}
					});
				}
			}
